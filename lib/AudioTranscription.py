...
class AudioTranscription:
    def __init__(
        ...
        print(
            f"Temps pour charger le modèle : {model_load_end - model_load_start:.2f} secondes"
        )

        # Trouver les périphériques d'entrée
        try:
            self.recording_device = self.find_device_by_name(
                self.RECORDING_DEVICE_NAME, kind="input"
            )
    ...
    def capture_and_transcribe(self):
        ...
        while True:
            ...
            if self.is_voice_detected(audio_data):
                ...
                if (
                    voice_detected
                    and time.time() - last_voice_time > self.SILENCE_DURATION
                ):
    ...
    def process_audio(self, recording):
        ...
        resampled_audio = resample_poly(
            audio_data, up=self.TARGET_SAMPLE_RATE, down=self.MIC_SAMPLE_RATE
        )
