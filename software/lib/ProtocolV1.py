...
class ProtocolV1(CanBusGsUsb):
    ...
    def read_pid_parameter_to_RAM(self, debug=None):
        ...
        msg = self.can_bus.sendMessage(
            ...
            data=[
                self.ADDR_READ_PID_PARAMETER_TO_RAM,
                0x00,
                0x00,
                0x00,
                0x00,
        ...
        real_current_loop_KP = msg.data[2] * (
            3 / 256
        )  # assuming that the maximum value of the current loop set by the system is 3
        real_current_loop_KI = msg.data[3] * (
            0.1 / 256
        )  # assuming that the maximum value of the current loop is 0.1
        real_speed_loop_KP = msg.data[4] * (
            0.1 / 256
        )  # assuming that the maximum value of the current loop is 0.1
        real_speed_loop_KI = msg.data[5] * (
            0.01 / 256
        )  # assuming that the maximum value of the current loop is 0.01
        real_position_loop_KP = msg.data[6] * (
            0.1 / 256
        )  # assuming that the maximum value of the current loop is 0.1
        real_position_loop_KI = msg.data[7] * (
            0.01 / 256
        )  # assuming that the maximum value of the current loop is 0.01
        ...
        if debug:
            print(
                f"Current loop KP parameters (0-256): {current_loop_KP} \t real: {real_current_loop_KP}"
            )
            print(
                f"Current loop KI parameters (0-256): {current_loop_KI} \t real: {real_current_loop_KI}"
            )
            print(
                f"Speed loop KP parameters (0-256): {speed_loop_KP} \t real: {real_speed_loop_KP}"
            )
            print(
                f"Speed loop KI parameters (0-256): {speed_loop_KI} \t real: {real_speed_loop_KI}"
            )
            print(
                f"Position loop KP parameters (0-256): {position_loop_KP} \t real: {real_position_loop_KP}"
            )
            print(
                f"Position loop KI parameters (0-256): {position_loop_KI} \t real: {real_position_loop_KI}"
            )
    ...
    def write_pid_parameter_to_RAM(
        self,
        current_loop_kp,
        current_loop_ki,
        speed_loop_kp,
        ...
        msg = self.can_bus.sendMessage(
            id=self.BASE_ADDR_ID + self.id,
            data=[
                self.ADDR_WRITE_PID_PARAMETER_TO_RAM,
                0x00,
                current_loop_kp,
                current_loop_ki,
                speed_loop_kp,
        ...
        if debug:
            print(
                f"Current loop KP parameters set to: {current_loop_KP} \t real: {real_current_loop_KP}"
            )
            print(
                f"Current loop KI parameters set to: {current_loop_KI} \t real: {real_current_loop_KI}"
            )
            print(
                f"Speed loop KP parameters set to: {speed_loop_KP} \t real: {real_speed_loop_KP}"
            )
            print(
                f"Speed loop KI parameters set to: {speed_loop_KI} \t real: {real_speed_loop_KI}"
            )
            print(
                f"Position loop KP parameters set to: {position_loop_KP} \t real: {real_position_loop_KP}"
            )
            print(
                f"Position loop KI parameters set to: {position_loop_KI} \t real: {real_position_loop_KI}"
            )
    ...
    def write_pid_parameter_to_ROM(
        self,
        current_loop_kp,
        current_loop_ki,
        speed_loop_kp,
        ...
        msg = self.can_bus.sendMessage(
            id=self.BASE_ADDR_ID + self.id,
            data=[
                self.ADDR_WRITE_PID_PARAMETER_TO_ROM,
                0x00,
                current_loop_kp,
                current_loop_ki,
                speed_loop_kp,
    ...
    def write_acceleration_to_RAM(self, acceleration, debug=None):
        ...
        if acceleration < 0 or acceleration > 0xFFFFFFFF:
            ...
            raise ValueError(
                "Invalid acceleration value. Acceleration must be in the range [0, 4294967295]dps/s"
            )
    ...
    def write_encoder_offset(self, offset, debug=None):
        ...
        if not isinstance(offset, int) or offset < 0 or offset > 65535:
            ...
            raise ValueError(
                "Invalid encoder offset value. Offset must be an integer in the range [0, 65535]"
            )

        msg = self.can_bus.sendMessage(
            id=self.BASE_ADDR_ID + self.id,
            data=[
                self.ADDR_WRITE_ENCODER_OFFSET,
                0x00,
                0x00,
                0x00,
                0x00,
    ...
    def write_current_position_to_ROM_as_motor_zero_position(self, debug=None):
        ...
        msg = self.can_bus.sendMessage(
            id=self.BASE_ADDR_ID + self.id,
            data=[
                self.ADDR_WRITE_CURRENT_POSITION_TO_ROM_AS_MOTOR_ZERO_POSITION,
                0x00,
                0x00,
                0x00,
                0x00,
    ...
    def read_multiturn_encoder_position(self, debug=None):
        ...
        if debug is not None:
            print(f"Encoder multiturn position: {motor_angle:.2f}°", end="\r")
        return motor_angle
    ...
    def read_singleturn_encoder_position(self, debug=None):
        ...
        msg = self.can_bus.sendMessage(
            id=self.BASE_ADDR_ID + self.id,
            data=[
                self.ADDR_READ_SINGLETURN_ENCODER_POSITION,
                0x00,
                0x00,
                0x00,
                0x00,
    ...
    def motor_off(self, debug=None):
        ...
        msg = self.can_bus.sendMessage(
            ...
            [self.ADDR_SHUTDOWN, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00],
        )
        if (
            msg.can_id == self.BASE_ADDR_ID + self.id
            and msg.data[0] == self.ADDR_SHUTDOWN
            and msg.data[1] == 0x00
            and msg.data[2] == 0x00
            and msg.data[3] == 0x00
            and msg.data[4] == 0x00
    ...
    def running(self, debug=None):
        ...
        if (
            msg.can_id == self.BASE_ADDR_ID + self.id
            and msg.data[0] == self.ADDR_RUNNING
            and msg.data[1] == 0x00
            and msg.data[2] == 0x00
            and msg.data[3] == 0x00
            and msg.data[4] == 0x00
    ...
    def read_clear_motor_error_flag(self, debug=None):
        ...
        msg = self.can_bus.sendMessage(
            id=self.BASE_ADDR_ID + self.id,
            data=[
                self.ADDR_READ_CLEAR_MOTOR_ERROR_FLAG,
                0x00,
                0x00,
                0x00,
                0x00,
    ...
    def position_closed_loop_control_1(self, position, debug=None):
        ...
        max_position = 2**32  # Maximum value for uint32_t
        tolerance = 0.2  # Tolerance in degrees
        ...
        self.can_bus.sendMessage(
            id=self.BASE_ADDR_ID + self.id,
            data=[
                self.ADDR_WRITE_POSITION_MULTITURN_CLOSED_LOOP_CONTROL,
                0x00,
                0x00,
                0x00,
                (position_control >> 0) & 0xFF,
                (position_control >> 8) & 0xFF,
                (position_control >> 16) & 0xFF,
                (position_control >> 24) & 0xFF,
            ],
        )

        while True:
            # Read the current motor angle and status
            ...
            # Handle overflow case (motor_angle near max_position and position near 0)
            if position == 0:
                # Adjust motor_angle for overflow
                if motor_angle > max_position - tolerance:
                    motor_angle = motor_angle - max_position

            # Check if the motor is within the target range and stopped
            if abs(motor_angle - position) < tolerance and motor_speed == 0.0:
                break
    ...
    def position_closed_loop_control_2(self, speed, position, debug=None):
        ...
        self.can_bus.sendMessage(
            id=self.BASE_ADDR_ID + self.id,
            data=[
                self.ADDR_WRITE_POSITION_MULTITURN_SPEED_CLOSED_LOOP_CONTROL,
                0x00,
                (max_speed >> 0) & 0xFF,
                (max_speed >> 8) & 0xFF,
    ...
    def position_closed_loop_control_3(self, direction, position, debug=None):
        ...
        - DATA[6-7]: Encoder position value (uint16_t type, 14-bit encoder value range 0 to 16383)
        """
        angle_control = int(position * 100)  # Convert position to 0.01°/LSB
        max_angle = 360.0  # Maximum angle for single-turn control
        tolerance = 0.5  # Tolerance in degrees

        # Send the position control command to the motor
        self.can_bus.sendMessage(
            id=self.BASE_ADDR_ID + self.id,
            data=[
                self.ADDR_WRITE_SINGLETURN_DIRECTION_CLOSED_LOOP_CONTROL,
                direction & 0xFF,  # Direction (0: CW, 1: CCW)
                0x00,
                0x00,
                (angle_control >> 0) & 0xFF,
                (angle_control >> 8) & 0xFF,
        ...
        while True:
            ...
            # Handle overflow case for single-turn (e.g., motor_angle near 360° and position near 0°)
            if abs(motor_angle - position) > max_angle - tolerance:
                # Normalize motor_angle to account for the overflow
                if motor_angle > position:
    ...
    def position_closed_loop_control_4(self, speed, direction, position, debug=None):
        ...
        - DATA[1]: Spin Direction byte (0x00 for clockwise, 0x01 for counterclockwise)
        - DATA[2-3]: Speed limit value (uint16_t type, maximum speed in dps)
        - DATA[4-5]: Position control value (uint16_t type, 0~35999, actual position is 0.01degree/LSB)
        - DATA[6-7]: NULL bytes
        ...
        - DATA[6-7]: Encoder position value (uint16_t type, 14-bit encoder value range 0 to 16383)
        """
        # Convert position to 0.01°/LSB (unit for the command)
        angle_control = int(position * 100)
        max_angle = 360.0  # Maximum angle for single-turn control
        tolerance = 0.5  # Tolerance in degrees for stopping condition

        # Validate inputs
        if not (0 <= position <= max_angle):
            raise ValueError(
                f"Invalid target position value: {position}. Must be in range [0, {max_angle}]°."
            )
        if not (0 < speed <= 1000):  # Example: limit speed to 1000 dps for safety
            raise ValueError(
                f"Invalid speed value: {speed}. Must be in range [1, 1000] dps."
            )
        if direction not in (0, 1):
            raise ValueError(
                f"Invalid direction value: {direction}. Must be 0 (CW) or 1 (CCW)."
            )

        # Send the position control command with speed limit
        self.can_bus.sendMessage(
            id=self.BASE_ADDR_ID + self.id,
            data=[
                self.ADDR_WRITE_SINGLETURN_SPEED_DIRECTION_CLOSED_LOOP_CONTROL,
                direction & 0xFF,  # Direction byte
                (speed >> 0) & 0xFF,
                (speed >> 8) & 0xFF,  # Speed limit
                (angle_control >> 0) & 0xFF,
                (angle_control >> 8) & 0xFF,  # Position target
                0x00,
                0x00,
            ],
        )

        # Loop until the motor reaches the target position
        while True:
            motor_angle = self.read_singleturn_encoder_position(debug=True)
            motor_temp, torque_current, motor_speed, encoder_position = (
                self.read_motor_status_2()
            ...
            # Handle overflow case for single-turn control
            if abs(motor_angle - position) > max_angle - tolerance:
                if motor_angle > position:
                    motor_angle -= max_angle
                else:
            ...
            # Check if the motor is within the target range and has stopped
            if abs(motor_angle - position) < tolerance and motor_speed == 0.0:
                break
